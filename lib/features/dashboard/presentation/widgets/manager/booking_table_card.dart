...
class BookingTableCard extends StatelessWidget {
  ...
  BookingTableCard({super.key, required this.dataList});
  ...
  Widget _buildTableRow(int rowNumber, bool isLastRow, Booking data) {
    return Container(
      decoration: BoxDecoration(
        border:
            isLastRow
      ...
      child: Table(
        ...
        children: [
          TableRow(
            children: [
              _tableCell(data.projectName),
              _tableCell(data.environmentName),
              _tableCell(formatDate(data.startDate ?? '')),
              _tableCell(formatDate(data.endDate ?? '')),
              Padding(
